...
public class PageUtil {
	...
	public String pageIndexList(int currentPage, int totalPage, String listUrl) {
		...
		if (totalPage > numPerBlock && currentPageSetup > 0) {// 전체페이지 > 뿌려지는 페이지 목록 개수 &그리고& 이전 페이지 번호 > 0 (1 2 3 4 5는
																// 이전 없기 때문)
		...
		// ◀이전 6 7 8 9 10 ▶
		// 5에서 6눌렀다고 가정
		// 6 <= 12(전체 페이지) && 6(누른 페이지) <= (5(이전페이지 눌렀을 때 가야할 페이지)+5(페이지블럭))) 즉 페이지 6부터
		// 시작해서 10까지 찍어라
		...
		while (page <= totalPage && page <= (currentPageSetup + numPerBlock)) {
			...
			if (page == currentPage) {
				...
				// 누적
				// sb.append("<font color=\"#4D71DB\">" + page + "</font>&nbsp;"); //내가보는 페이지는
				// 색깔만 입히고 링크 안걸기
				...
				// sb.append("<a href=\"" + listUrl + "pageNum=" + page + "\"><font
				// color=\"#696969\">" + page + "</font></a>&nbsp;"); //그 외는 링크 걸기 -> get방식으로
				// 다이렉트로 전달하여 페이지 이동
